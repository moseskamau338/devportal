<template>
  <section class="relative">
    <div class="absolute w-full flex justify-end -mt-10">
      <button @click="uiStore.minimize_recon_stats = !uiStore.minimize_recon_stats" class="text-gray-400 hover:text-gray-500 transition-all text-xs">
        <i :class="[uiStore.minimize_recon_stats? 'fa-plus-circle':'fa-minus-circle']" class="fa-solid"></i>
        {{ uiStore.minimize_recon_stats? 'Show stats' : 'Minimize' }}
      </button>
    </div>

    <div v-if="!uiStore.minimize_recon_stats" class="grid grid-cols-1 lg:grid-cols-12 gap-4 ">
      <!--status summary-->
      ...
      <div class="grid grid-cols-1 xl:grid-cols-12 xl:w-[70%] bg-white dark:bg-churpy-night-box shadow-md rounded p-4">
        <div class="lg:col-span-6">
          ...
          <div class="flex flex-wrap whitespace-pre-wrap mt-4 md:space-y-reverse space-x-1">
            <c-button disabled variant="success" class="my-2 lg:my-0">
              <i class="fa-duotone fa-layer-group mr-1"></i>
             Bulk Match
           </c-button>
            <c-button disabled variant="warning" class="my-2 lg:my-0">
...
export default {
  ...
  setup(){
    ...
    return {period, status_summary, categories, currency:helpers.currency, uiStore}
  }
}
</script>