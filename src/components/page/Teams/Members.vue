<template>
  ...
  <div class="flex flex-col md:flex-row md:justify-between md:items-center space-y-3 md:space-y-reverse mt-10">
    ...
    <div id="left-actions" class="flex">
      ...
      <input type="text" name="query" id="text" class="focus:ring-green-500 focus:border-green-500 block w-[90%] focus:w-full pl-10 sm:text-sm border-gray-300 rounded-md transition-all dark:bg-churpy-night dark:border-gray-600" placeholder="Search members" />
    </div>
    </div>
  </div>
        ...
        <div class="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
          <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
              <thead class="bg-gray-50 dark:bg-churpy-night">
                <tr>
                  <th v-for="(head, index) in ['Name', 'Phone', 'Email', 'Role', 'Actions']" :key="index"  scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 dark:text-gray-400">
              ...
              <tbody class="divide-y divide-gray-200 bg-white dark:bg-gray-700">
                <tr v-for="(member, index) in members" :key="index">
                  <td class="whitespace-nowrap flex items-center py-4 pl-4 pr-3 text-sm font-medium text-gray-700 dark:text-gray-300 sm:pl-6">
                  ...
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">{{ member.phone }}</td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">{{ member.email }}</td>
                  <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-500 dark:text-gray-300">

                    <Listbox as="div" v-model="member.role">
                      <div class="mt-1 relative">
                        <ListboxButton class="bg-white dark:bg-churpy-night relative w-full border border-gray-300 rounded shadow-sm pl-3 pr-10 py-1 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-churpy-green focus:ring-green-500 focus:border-green-500 sm:text-sm">
                        ...
                        <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100" leave-to-class="opacity-0">
                          <ListboxOptions class="absolute z-10 mt-1 w-full bg-white dark:bg-churpy-night dark:border dark:border-gray-500 shadow-md max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm">
