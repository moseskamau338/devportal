    ...
    <div class="hidden md:flex md:w-42 md:flex-col md:fixed md:inset-y-0">
      ...
      <div class="flex flex-col flex-grow pt-5 bg-churpy-dark overflow-y-auto">
        ...
        <div class="mt-5 flex-1 flex flex-col">
            ...
            <router-link v-for="item in navigation" :key="item.name" :to="item.href" :class="[$route.name === item.route_name ? 'bg-gradient-to-br from-churpy-green via-green-600 to-emerald-600 text-white shadow-sm shadow-churpy-green' : 'text-indigo-100 hover:bg-green-400/30', 'group flex items-center px-2 py-1 text-sm font-medium rounded-md']">
                <i :class="item.icon" class="fa-solid mr-3 flex-shrink-0 text-indigo-100"></i>
                {{ item.name }}
            </router-link>
            </nav>
...
const navigation = [
  { route_name:'home',name: 'Dashboard', href: '/', icon: 'fa-solid fa-objects-column', current: true },
  { route_name:'teams',name: 'Team', href: '/teams', icon: 'fa-solid fa-users', current: false },
  { route_name:'projects',name: 'Projects', href: '#', icon: 'fa-solid fa-folder-grid', current: false },
]
